...
public class NotifyBoard {
    ...
    public void whenTextFigureCreated(StickyNoteCreatedDomainEvent stickyNoteCreatedDomainEvent) {
        ...
        if (board.isPresent()) {
            board.get().commitTextFigure(stickyNoteCreatedDomainEvent.getTextfigureId());
            boardRepository.save(board.get());
            domainEventBus.post(new TextfigureCommittedDomainEvent(stickyNoteCreatedDomainEvent.getBoardId(), stickyNoteCreatedDomainEvent.getTextfigureId()));
        } else {
